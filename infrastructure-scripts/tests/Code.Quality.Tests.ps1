$Scripts = Get-ChildItem -Path $PSScriptRoot\..\*.ps1 -File

Describe "Script documentation tests" {
...
Describe "Script code quality tests" {
...
Describe "Script has associated test tests" {

    foreach ($Script in $Scripts) {
        $TestName = "$($Script.BaseName).Tests.ps1"
        Context "$($Script.BaseName)" {
            It "Should have an associated test called $TestName" {
