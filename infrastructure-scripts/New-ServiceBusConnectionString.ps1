...
try {
    ...
    if (!$AuthorizationRule) {
        Write-Host "Creating authorization rule with name $AuthorizationRuleName"
    ...
    Write-Host "Retrieving $ConnectionStringType connection string"
    ...
    Write-Host "Publishing VSTS variable: ServiceBusConnectionString"
