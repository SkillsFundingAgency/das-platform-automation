...
function Invoke-SqlAddGrants {
    ...
    foreach ($Grant in $Grants) {
        ...
        if ($GrantObject) {
            Write-Host "      -> Granting $GrantName on $GrantObject"
...
function Invoke-SqlLogRedundantRoles {
    ...
    $SqlCmdParameters = @{
        ...
        Variable          = @("Username=$Username","Roles=$RolesList")
    }

    $Result = Invoke-Sqlcmd @SqlCmdParameters

    if ($Result) {
        $RedundantDatabaseRoles = $Result.Role -join ', '
...
function Invoke-SqlLogRedundantGrants {
    ...
    foreach ($Grant in $Grants) {
        ...
        $GrantObject = ($Grant -split "-")[1]
        if ($GrantObject) {
            $GrantFound = $DatabaseGrants | Where-Object { $_.name -eq $Username -and $_.permission_name -eq $GrantName -and $_.schema_object -eq $GrantObject }
        }
        else {
            $GrantFound = $DatabaseGrants | Where-Object { $_.name -eq $Username -and $_.permission_name -eq $GrantName -and $_.class_desc -eq "DATABASE" }
        }
